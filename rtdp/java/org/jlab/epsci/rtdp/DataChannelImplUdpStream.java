...
import org.jlab.coda.emu.support.data.tuple.Quartet;
...
public class DataChannelImplUdpStream extends DataChannelAdapter {
    ...
    private final int DEFAULT_MTU = 9000;
    ...
    /**
     * Constructor to create a new DataChannelImplUdpStream instance.
     *
...
//System.out.println("    DataChannel UDP stream: dumpData = " + dumpData);
            ...
            // Let's try 20MB buf by default, which will most likely get doubled to 40MB by the system
            sendBufSize = 20000000;

            // Host to use for a sending UDP socket (ejfat-3-daq)
            destHost = "129.57.177.3";
...
//System.out.println("    DataChannel UDP stream: end/reset(), interrupt parser/merger thread");
...
//System.out.println("    DataChannel UDP stream: end/reset(), interrupt input thread");
        }
...
//System.out.println("    DataChannel UDP stream: end/reset(), joined parser/merger thread");

            try {
                dataInputThread.join(1000);
            }
            catch (InterruptedException e) {}
//System.out.println("    DataChannel UDP stream: end/reset(), joined input thread");
        }
    }
    ...
    private final class DataInputHelper extends Thread {
        ...
        public void run() {
            ...
            latch.countDown();
            System.out.println("    DataChannel UDP stream in: " + name + " - started");
...
//                    Utilities.printBytes(buffer, 0, 100, "Streamed buf");
                ...
                System.out.println("    DataChannel UDP stream in: " + name + ", interrupted, exit reading thd");
            }
            catch (AsynchronousCloseException e) {
                System.out.println("    DataChannel UDP stream in: " + name + ", socket closed, exit reading thd");
            }
            catch (IOException e) {
                // Assume that if the other end of the socket closes, it's because it has
                // sent the END event and received the end() command.
                System.out.println("    DataChannel UDP stream in: " + name + ", socket I/O error");
            }
            catch (Exception e) {
            ...
            try {
                ...
                while (true) {
                    ...
                    if (parseStreamingToRing(item, bbSupply)) {
                        System.out.println("    DataChannel UDP stream in: 1 quit streaming parser/merger thread for END event from " + name);
                        break;
                    }
                }
            }
...
//System.out.println("    DataChannel UDP stream in: buildable has frame = " + frame + ", timestamp = " + timestamp + ", pos = " + pos);
                    ...
                    System.out.println("    DataChannel UDP stream in: got " + controlType + " event from " + name);
                    if (controlType == null) {
                        System.out.println("    DataChannel UDP stream in: found unidentified control event");
                    ...
                    isUser = true;
                    if (hasFirstEvent) {
                        System.out.println("    DataChannel UDP stream in: " + name + " got FIRST event");
                    }
                    else {
                        System.out.println("    DataChannel UDP stream in: " + name + " got USER event");
                    }
                }
...
//System.out.println("    DataChannel UDP stream in: put buildable event into channel ring, event from " + name);
                    ri.setAll(null, null, node, evType, controlType,
                            isUser, hasFirstEvent, isStreamingData, id, recordId, id,
                            node.getNum(), name, item, bbSupply);
                    ri.setTimeFrame(frame);
                    ri.setTimestamp(timestamp);
                }
                else {
//System.out.println("    DataChannel UDP stream in: put CONTROL (user?) event into channel ring, event from " + name);
                    ri.setAll(null, null, node, evType, controlType,
                            isUser, hasFirstEvent, isStreamingData, id, recordId, id,
                ...
                if (controlType == ControlType.END) {
                    ...
                    System.out.println("    DataChannel UDP stream in: BREAK from loop, got END event");
                    break;
                }
            }
    ...
    private final class DataOutputHelper extends Thread {
        ...
        private final class SocketSender extends Thread {
            ...
            SocketSender(ByteBufferSupply bufSupply, InetAddress addr, int port) {
                ...
                destAddr = addr;
                destPort = port;
System.out.println("    DataChannel UDP out: constructed SocketSender thread");
            }
...
//System.out.println("    DataChannel UDP stream out: get BB from BBsupply");
...
//System.out.println("    DataChannel UDP stream out: " + name + " - send packetized buffer of len " + buf.limit());
...
//System.out.println("    DataChannel UDP out: connect UDP socket to dest " + destAddr.getHostName() + " port " + port);
                if (useConnectedSocket) {
...
//System.out.println("    DataChannel UDP stream: LB header: tick = " + tick + ", entropy = " + entropy);
...
System.out.println("    DataChannel UDP stream out " + outputIndex + ": send PRESTART event");
                            gotPrestart = true;
...
System.out.println("    DataChannel UDP stream out " + outputIndex + ": send " + pBankControlType + " event");
                            writeEvioData(ringItem);
...
System.out.println("    DataChannel UDP stream out " + outputIndex + ": send user event");
                        // Write user event
                        writeEvioData(ringItem);
                    }
...
System.out.println("    DataChannel UDP stream out: " + name + " got END event, quitting 1");
...
//System.out.println("    DataChannel UDP stream out: send seq " + nextSequences[ringIndex] + ", release ring item");
                    ...
                    if (outputRingCount > 1 && pBankControlType == null && !pBankType.isUser()) {
                        setNextEventAndRing();
//System.out.println("    DataChannel UDP stream out, " + name + ": for seq " + nextSequences[ringIndex] + " SWITCH TO ring = " + ringIndex);
                    }

                    if (pBankControlType == ControlType.END) {
                        // END event automatically flushed in writeEvioData()
System.out.println("    DataChannel UDP stream out: " + name + " got END event, quitting 2");
                        threadState = ThreadState.DONE;
                        return;
                    }
                    ...
                    // If I've been told to RESET ...
                    if (gotResetCmd) {
System.out.println("    DataChannel UDP stream out: " + name + " got RESET cmd, quitting");
                        threadState = ThreadState.DONE;
                        return;
                    }
...
//        ", delta = " + (t - lastSendTime));
                ...
                System.out.println("    DataChannel UDP stream out: " + name + "  interrupted thd, quitting");
            }
            catch (Exception e) {
                e.printStackTrace();
