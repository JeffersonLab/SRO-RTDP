...
#-----------------------------------------------------
    def emulate(self, login_pause=False, emu_config="emulate.yaml", sleep_time=30):
        ...
        login_pause : boolean, optional
            whether to pause for man-in-the-loop logins
    ...
    def parse_emu_logs(self, retrieve=True):
        ...
        for index, cmpnt_id in enumerate(cmpnt_ids):
            ...
            for line in frame_sz_lines:
                ...
                #################################################################################################################
        # Extract lines with latency information all components
        for index, cmpnt_id in enumerate(cmpnt_ids):
            fs_value = pd.NA
            fn_value = pd.NA
            cl_value = pd.NA
        ...
        # Extract lines with drop information all components
        for index, cmpnt_id in enumerate(cmpnt_ids):
            fs_value = pd.NA
            ts_value = pd.NA
            fn_value = pd.NA
